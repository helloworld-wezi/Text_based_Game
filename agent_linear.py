...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() < epsilon:
        action_index, object_index = index2tuple(np.random.randint(0, NUM_ACTIONS * NUM_OBJECTS))
    else:
        choosen_q_val = (theta @ state_vector)[tuple2index(0, 0)]
        action_index, object_index = 0, 0
        for i in range(NUM_ACTIONS):
            for j in range(NUM_OBJECTS):
                q_value = (theta @ state_vector)[tuple2index(i, j)]
                if q_value > choosen_q_val:
                    choosen_q_val = q_value
                    action_index = i
                    object_index = j
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    max_next_q_val = (theta @ next_state_vector)[tuple2index(0, 0)]
    for i in range(NUM_ACTIONS):
        for j in range(NUM_OBJECTS):
            q_value = (theta @ next_state_vector)[tuple2index(i, j)]
            if q_value > max_next_q_val:
                max_next_q_val = q_value

    current_idx = tuple2index(action_index, object_index)

    if terminal:
        update = ALPHA * (reward - (theta @ current_state_vector)[
            tuple2index(action_index, object_index)]) * current_state_vector
        # TODO Your update here
    else:
        update = ALPHA * (reward + GAMMA * max_next_q_val - (theta @ current_state_vector)[
            tuple2index(action_index, object_index)]) * current_state_vector
        # TODO Your update here

    theta[current_idx] = theta[current_idx] + update
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            ...
            # TODO Your code here
            # pass
            linear_q_learning(theta, current_state_vector, action_idx, object_idx, reward, next_state_vector, terminal)
