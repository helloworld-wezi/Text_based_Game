...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        action_index = torch.argmax(q_values_action)
        object_index = torch.argmax(q_values_object)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    if terminal:
        ...
        target = torch.FloatTensor(reward + GAMMA * maxq_next)

    loss = 0.5 * (target - 1 / 2 * (q_value_cur_state[0][action_index] + q_value_cur_state[1][
        object_index])) ** 2  # need more contemplation on this
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            ...
            # TODO Your code here
            # pass
            deep_q_learning(current_state_vector, action_idx, object_idx,
